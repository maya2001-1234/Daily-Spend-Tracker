 ...
 */
export function getMonthlyStats(expenses) {
  const groups = {};
  ...
  expenses.forEach((expense) => {
    ...
    groups[monthKey].byCategory[expense.category] =
      (groups[monthKey].byCategory[expense.category] || 0) + expense.amount;
  });
...
export function rateMonthlySpending(
  ...
  const targetStats = allMonthlyStats.find(
    (m) => m.monthKey === targetMonthKey
  );

  const targetTotal = targetStats ? targetStats.total : 0;

  const earlierStats = allMonthlyStats.filter(
    (m) => m.monthKey < targetMonthKey
  );
  ...
  if (earlierStats.length === 0) {
    return {
      ...
      percentageDiff: 0,
      advice:
        'This is either your earliest recorded month or starting period. Log data in other months to generate dynamic financial flow ratings!',
    };
  }

  const sumEarlier = earlierStats.reduce((sum, s) => sum + s.total, 0);
  ...
  if (avgEarlier === 0) {
    return {
      ...
      percentageDiff: 0,
      advice:
        'Historical months represent zero expenditures. Add past spends to get month-over-month performance ratings.',
    };
  }
  ...
  if (percentageDiff <= -15) {
    ...
    return {
      ...
      percentageDiff,
      advice: `Fantastic progress! Your spending this month is ${Math.abs(
        Math.round(percentageDiff)
      )}% lower than your typical historical average of ${formatMoney(
        avgEarlier,
        currency
      )}. You are building strong financial muscle!`,
    };
    ...
    return {
      ...
      percentageDiff,
      advice: `Congratulation! You are pacing ${Math.abs(
        Math.round(percentageDiff)
      )}% below your historical averages (${formatMoney(
        avgEarlier,
        currency
      )}). Keep managing your discretionary spending!`,
    };
  } else if (percentageDiff <= 5) {
    ...
    return {
      ...
      percentageDiff,
      advice: `Your spend flow is in perfect alignment with historical patterns (${formatMoney(
        avgEarlier,
        currency
      )}). You are tracking standard expenditures without significant variance.`,
    };
    ...
    return {
      ...
      percentageDiff,
      advice: `Caution: Spending is ${Math.round(
        percentageDiff
      )}% above your historical average (${formatMoney(
        avgEarlier,
        currency
      )}). Investigate your shopping or travel bills to locate the leak.`,
    };
  } else {
    const score = Math.max(
      10,
      Math.round(30 - (percentageDiff - 20) / 2)
    );
    ...
    return {
      ...
      percentageDiff,
      advice: `Alert: Spending has shot up by ${Math.round(
        percentageDiff
      )}% compared to your historical average which sits at ${formatMoney(
        avgEarlier,
        currency
      )}. It is highly advised to cut non-essential items immediately!`,
    };
  }
}